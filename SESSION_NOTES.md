...
## ACTIVE TASK
**Task:** **Session 16 DONE — Phase 7 (v2): diagram preview webview** (`Quarto: Preview Diagram` / `quarto.previewDiagram`). Third v2 webview feature shipped + adversarial-review-hardened. There is NO forced next deliverable — **WAIT for the operator to pick** (remaining Phase 7 slices, Phase 6d/6e, a Polish item, or the operator-only `vsce publish`). *Background: v1 is feature-complete + release prep is AGENT-COMPLETE (Sessions 1–13, only operator-only `vsce publish` remains); Session 14 shipped formatting toggles, Session 15 math preview.*
**Status:** Diagram preview complete + hardened. Baselines now **242 unit + 63 integration** green (was 226/57); clean **33-file** `.vsix` (**1.29 MB packaged** — the +~0.9 MB over Session 15's 368.5 KB is the vendored Mermaid renderer in `media/mermaid/`, a 3.56 MB UMD asset that compresses ~3×). `npm audit` unchanged = 7 dev-only vulns (Mermaid added **0** — vendored as a SINGLE file, NOT a devDep; Learning #23d). Repo PUBLIC; `origin` → `rmsharp/vscode_quarto_ext` (branch `master`). **Local `master` is ahead of `origin` — this session's commits are unpushed** (operator pushes; see gotchas).
...
**Priority:** v2 in progress. Next operator pick from `BACKLOG.md` "Up Next" (Phase 6d/6e; remaining Phase 7 slices: snippets, image paste, graphviz-rendering follow-up) or "Polish / deferred".
**⚠ STRICT TDD IS MANDATORY** for any code/bugfix (operator directive — `CLAUDE.md` §"Mandatory development practice" + Learnings #10, #14, #15, #16, #21, #22, **#23**). Pure config/grammar/doc edits with no logic are exempt but still need their normal verification (compile, package, AND — per **Learning #18** — `npm run test:integration` after any `publisher`/`name`/activation change; the 8 suites hard-code the extension ID).
...
2. **v2 features** (`BACKLOG.md` "Up Next"): Phase 6d/6e (YAML/cell-option completion 🐉needs a YAML-schema-source decision — likely a spike/plan first; embedded-cell language completion 🐉offset-mapping risk) + the remaining **Phase 7** authoring-aid slices (snippets, image paste, and the **graphviz-rendering follow-up** to this session's diagram preview — Learning #23f). Real implementation → **strict TDD** (`/tdd`), §3.3 pure-`core/` guardrail, vitest + `@vscode/test-electron`. **Reuse the established patterns** — the formatting-toggle editor-mutation shape (Learning #21) and the **webview shape** (Learnings #22/#23: pure detection + pure HTML/CSP builder + thin adapter, vendored MIT asset, faithful tabGroups verification; #23 adds the large-pre-bundled-lib vendoring + diagram-CSP nuances).
3. **Polish / deferred** (`BACKLOG.md` "Polish / deferred", each its own session): **multi-cursor formatting**; **mid-line HTML comments in `scanRegions`** (new this session — shared-model gap, affects math-regions + refs); the dev-toolchain upgrade (Learning #20); de-dup the 8-copy `EXTENSION_ID` constant (Learning #18); indented-code-block phantom; setext headings.
