...
## ACTIVE TASK
**Task:** **Session 19 DONE — Phase 6d Slice 6d-2 (`#|` cell-option VALUE completion)**. SHIPPED + adversarial-review-hardened. After a known cell-option key's colon, typing on a `#|`/`//|` line now offers that option's value enum (curated; booleans, echo+fenced, output+asis, code-fold+show, fig-align, the 18-value page-column enum). The runtime schema reader is the later 6d-3. **Recommended next deliverable: implement Slice 6d-3** (the schema reader — runtime-read the user's Quarto schema, degrade to the curated fallback) — but the operator may pick otherwise (6d-4/6d-5 front matter, a Phase 7 slice, 6e, a Polish item, or the operator-only `vsce publish`). *Background: v1 feature-complete + release prep AGENT-COMPLETE (Sessions 1–13); 14 formatting toggles, 15 math preview, 16 diagram preview, 17 6d plan, 18 6d-1.*
**Status:** Shipped + committed (8 commits incl. the standalone Phase 1B stub + 4 layer-checkpoint feature commits + 2 review-fix commits). Baselines now **297 unit + 79 integration** green (was 273/70); clean **33-file** `.vsix` (1.29 MB — new core is bundled). `npm audit` unchanged = 7 dev-only vulns (Learning #20). §3.3 guardrail intact (no `vscode` import in `core/`). Repo PUBLIC; `origin` → `rmsharp/vscode_quarto_ext` (branch `master`). **`master` is AHEAD of `origin` by Session 17–18's commits + this session's ~9 — confirm with `git status`; pushing is the operator's call** (prior sessions left it to the operator).
**Plan:** **6d is governed by `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md`** (per-phase plan). 6d-1 + 6d-2 are done; **6d-3** is next (plan §6 — the dragon slice). Other v2 phases (Phase 7 remaining slices, 6e) are implemented directly from the parent plan. Strict TDD throughout.
**Priority:** Implement Phase 6d, one slice per session, **next is 6d-3** (plan §6). Each slice = strict TDD, §3.3 pure-`core/` guardrail, vitest + `@vscode/test-electron`.
**⚠ STRICT TDD IS MANDATORY** for any code/bugfix (operator directive — `CLAUDE.md` §"Mandatory development practice" + Learnings #10, #14, #15, #16, #21, #22, #23, #24, #25, **#26**). Pure config/grammar/doc edits with no logic are exempt but still need their normal verification (compile, package, AND — per **Learning #18** — `npm run test:integration` after any `publisher`/`name`/activation change; the suites hard-code the extension ID).

### What You Must Do (recommended: implement Slice 6d-3; operator may redirect)
**Read `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md` §6 (Slice 6d-3) first — this is the 🐉 dragon slice; give it full attention.** 6d-3 = the **schema reader**: replace the curated cell-option source with the **full** schema read from the user's installed Quarto, degrading to the curated set on any failure. New work: add `quartoSharePath()` to `src/quarto/cli.ts` (spawns the **undocumented** `quarto --paths`, parse **defensively** — last non-empty stdout line / the existing dir ending in `share`; add it to the "re-verify on Quarto upgrade" marker list); add a pure, **never-throw** `parseSchemaIndex(jsonText)` to `src/core/yaml-schema.ts` that parses `<share>/editor/tools/yaml/yaml-intelligence-resources.json` (BOM-strip, try/catch→fallback; resolve **simple** `schema`→values only — `boolean`→[true,false], `{enum}`, `{anyOf}`-of-those, `maybeArrayOf{enum}`; resolve `{ref}` into the same file's `schema/definitions.yml` ONLY for the simple cases like `page-column`; **defer** deep `ref`/`super`/`resolveRef`); exclude `hidden:true`; read `tags.engine` for the knitr/jupyter filter; add an adapter `src/features/yaml-schema-source.ts` (spawn + `fs.readFile` + cache keyed by `quarto --version` + fallback to `CURATED_CELL_OPTIONS`); wire the provider to query the index. **Faithful degradation test (gate d):** temporarily break `quartoSharePath` → the curated set still serves → revert (break-revert, Learning #16). **Do NOT vendor** (runtime-read needs no NOTICE). **One slice, close out** (FM #18 — do NOT also do 6d-4/front matter). If the operator instead wants a non-6d item: Phase 7 slices (snippets, image paste, graphviz `{dot}` rendering — Learning #23f), Phase 6e (🐉 offset-mapping), a Polish item (BACKLOG), or the operator-only `vsce publish`.
