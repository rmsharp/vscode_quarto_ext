...
## ACTIVE TASK
**Task:** **Session 18 DONE — Phase 6d Slice 6d-1 (`#|` cell-option KEY completion)**. SHIPPED + adversarial-review-hardened. Typing on a `#|`/`//|` option line inside an executable cell now offers cell-option key suggestions (curated set; the runtime schema reader is the later 6d-3). **Recommended next deliverable: implement Slice 6d-2** (`#|` cell-option VALUE completion — enum/boolean, still curated) — but the operator may pick otherwise (other 6d slices are sequential; or a Phase 7 slice, 6e, a Polish item, or the operator-only `vsce publish`). *Background: v1 feature-complete + release prep AGENT-COMPLETE (Sessions 1–13); 14 formatting toggles, 15 math preview, 16 diagram preview, 17 6d plan.*
**Status:** Shipped + committed (5 commits incl. the Phase 1B stub + the review-fix commit). Baselines now **273 unit + 70 integration** green (was 242/63); clean **33-file** `.vsix` (1.29 MB — new core is bundled, fixture excluded). `npm audit` unchanged = 7 dev-only vulns (Learning #20). §3.3 guardrail intact (no `vscode` import in `core/`). Repo PUBLIC; `origin` → `rmsharp/vscode_quarto_ext` (branch `master`). **`master` is AHEAD of `origin` by Session 17's 2 commits + this session's 5 — confirm with `git status`; pushing is the operator's call** (prior sessions left it to the operator).
**Plan:** **6d is governed by `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md`** (per-phase plan). 6d-1 is done; **6d-2** is next (plan §6). Other v2 phases (Phase 7 remaining slices, 6e) are implemented directly from the parent plan. Strict TDD throughout.
**Priority:** Implement Phase 6d, one slice per session, **next is 6d-2** (plan §6). Each slice = strict TDD, §3.3 pure-`core/` guardrail, vitest + `@vscode/test-electron`.
**⚠ STRICT TDD IS MANDATORY** for any code/bugfix (operator directive — `CLAUDE.md` §"Mandatory development practice" + Learnings #10, #14, #15, #16, #21, #22, #23, #24, **#25**). Pure config/grammar/doc edits with no logic are exempt but still need their normal verification (compile, package, AND — per **Learning #18** — `npm run test:integration` after any `publisher`/`name`/activation change; the suites hard-code the extension ID).

### What You Must Do (recommended: implement Slice 6d-2; operator may redirect)
**Read `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md` §6 (Slice 6d-2) first.** 6d-2 = `#|` cell-option **VALUE** completion (after `#| echo: ` offer `true`/`false`; after `#| output: ` offer `true`/`false`/`asis`/…), from **curated enums** for the common options (still no runtime reader — that's 6d-3). New work: extend `core/yaml-context.ts` `completionContextAt` to detect a `cell-option-value` position (cursor PAST the colon — the slot it currently returns `null` for), add a value-slot to `CellOptionLine` in the model, add curated `values` to the `CURATED_CELL_OPTIONS` fields in `core/yaml-schema.ts`, handle the value kind + the `:` trigger in `providers/yaml.ts`, and add unit + integration tests (incl. the inverted-gating regression: no value items mid-key / on prose). **Reuse the 6d-1 shape** (pure-detection + pure-data + thin-adapter; the `replaceRange`/`{inserting,replacing}` contract; the `detail`-discriminator no-cross-pollination test with positive+negative controls). **One slice, close out** (FM #18 — do NOT also do 6d-3/the reader). If the operator instead wants a non-6d item: Phase 7 slices (snippets, image paste, graphviz `{dot}` rendering — Learning #23f), Phase 6e (🐉 offset-mapping), a Polish item (BACKLOG), or the operator-only `vsce publish`.

### Useful starting context
- **★ Phase 6d-1 SHIPPED (Session 18, → Learning #25) — the 6d-2 executor reuses all of this:** `src/core/qmd/model.ts` `findCellOptionLines(text)` (a view over `findAllCells`; returns `{line, cellLang, prefix, keySlot}` — `keySlot` is the key-token span or `null`; `CELL_OPTION_PREFIX` at `model.ts:319` now matches Quarto's real `^#\s*\| ?`); `src/core/yaml-context.ts` `completionContextAt(text, offset)` (returns a `cell-option-key` `YamlCompletionContext` or `null` — **6d-2 extends this for the value position it currently returns null on, PAST the colon**); `src/core/yaml-schema.ts` `SchemaField` + `CURATED_CELL_OPTIONS` (~18, grounded names + own descriptions — **6d-2 adds `values`**); `src/providers/yaml.ts` `YamlCompletionProvider` (gated to cell-option-key, triggers `| : -`, items tagged `detail:"Quarto cell option"`); wired in `extension.ts:38`. Tests: `test/unit/cell-option-lines.test.ts` (16), `test/unit/yaml-context.test.ts` (11), `test/unit/yaml-schema.test.ts` (4), `test/integration/suite/yaml.test.ts` (7), fixture `test/fixtures/yaml-completion.qmd`.
