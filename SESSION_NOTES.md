...
## ACTIVE TASK
**Task:** **Session 17 DONE — Phase 6d spike + implementation plan** (YAML/`#|` cell-option completion). PLANNING session, no code. The 🐉 schema-source dragon is **resolved** and 6d is **decomposed into 5 vertical slices** in `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md`. **Recommended next deliverable: implement Slice 6d-1** (`#|` cell-option KEY completion, the smallest slice) — but the operator may pick otherwise (other 6d slices are sequential; or a Phase 7 slice, 6e, a Polish item, or the operator-only `vsce publish`). *Background: v1 feature-complete + release prep AGENT-COMPLETE (Sessions 1–13); Session 14 formatting toggles, 15 math preview, 16 diagram preview.*
**Status:** Plan written + committed. **No code changed** — baselines unchanged at **242 unit + 63 integration** green; clean **33-file** `.vsix` (1.29 MB). `npm audit` unchanged = 7 dev-only vulns (Learning #20). Repo PUBLIC; `origin` → `rmsharp/vscode_quarto_ext` (branch `master`). **This session's commit may be unpushed — confirm `git status`** (prior sessions left pushing to the operator).
**Plan:** **6d is now governed by `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md`** (the per-phase plan; the parent `docs/planning/2026-06-27-extension-architecture-plan.md` §6 deferred 6d to "flag at planning of that session"). Other v2 phases (Phase 7 remaining slices, 6e) are still implemented directly from the parent plan. Strict TDD throughout.
**Priority:** Implement Phase 6d, one slice per session, **starting with 6d-1** (plan §6). Each slice = strict TDD, §3.3 pure-`core/` guardrail, vitest + `@vscode/test-electron`.
**⚠ STRICT TDD IS MANDATORY** for any code/bugfix (operator directive — `CLAUDE.md` §"Mandatory development practice" + Learnings #10, #14, #15, #16, #21, #22, #23, **#24**). Pure config/grammar/doc edits with no logic are exempt but still need their normal verification (compile, package, AND — per **Learning #18** — `npm run test:integration` after any `publisher`/`name`/activation change; the suites hard-code the extension ID).

### What You Must Do (recommended: implement Slice 6d-1; operator may redirect)
**Read `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md` first** — it is the deliverable of Session 17 and carries everything: the resolved schema-source decision (§2), the slice sequence (§6), the grep-verified reuse/gaps inventory (§4, file:line), the pure-core interface contracts (§5), the risks incl. the inverted provider-gating trap (§4.3, §7), and the open questions for the executor (§9). **Do NOT re-derive the dragon — it's resolved.** Slice 6d-1 scope (smallest, ship first): `#|` cell-option KEY completion from a **curated** set (no runtime schema reader yet — that's 6d-3). New work: `findCellOptionLines` in the shared model (Gap #2), `core/yaml-context.ts` `completionContextAt` (cell-option-key only), curated `CURATED_CELL_OPTIONS`, `providers/yaml.ts` gated to cell-option lines + the no-cross-pollination regression test, wire in `extension.ts`. **One slice, close out** (FM #18 — do NOT also do 6d-2/values). If the operator instead wants a non-6d item, the options are: Phase 7 slices (snippets, image paste, graphviz `{dot}` rendering follow-up — Learning #23f), Phase 6e (🐉 offset-mapping), a Polish item (BACKLOG), or the operator-only `vsce publish`.
