...
## ACTIVE TASK
**Task:** **Session 15 DONE — Phase 7 (v2): math preview webview** (`Quarto: Preview Math` / `quarto.previewMath`). Second v2 feature shipped + adversarial-review-hardened. There is NO forced next deliverable — **WAIT for the operator to pick** (more Phase 7 slices, Phase 6d/6e, a Polish item, or the operator-only `vsce publish`). *Background: v1 is feature-complete + release prep is AGENT-COMPLETE (Sessions 1–13, only operator-only `vsce publish` remains); Session 14 shipped formatting toggles.*
**Status:** Math preview complete + hardened. Baselines now **226 unit + 57 integration** green (was 202/51); clean **32-file** `.vsix` (368.5 KB — the +338 KB over Session 14's 29.82 KB is the vendored KaTeX renderer in `media/katex/`). `npm audit` unchanged = 7 dev-only vulns (katex added **0**; accepted, `docs/SECURITY-AUDIT.md`). Repo PUBLIC; `origin` → `rmsharp/vscode_quarto_ext` (branch `master`). **Local `master` is ahead of `origin` — this session's commits are unpushed** (operator pushes; see gotchas).
...
**Priority:** v2 in progress. Next operator pick from `BACKLOG.md` "Up Next" (Phase 6d/6e; remaining Phase 7 slices: snippets, diagram preview, image paste) or "Polish / deferred".
...
2. **v2 features** (`BACKLOG.md` "Up Next"): Phase 6d/6e (YAML/cell-option completion 🐉needs a YAML-schema-source decision — likely a spike/plan first; embedded-cell language completion 🐉offset-mapping risk) + the remaining **Phase 7** authoring-aid slices (snippets, diagram preview, image paste). Real implementation → **strict TDD** (`/tdd`), §3.3 pure-`core/` guardrail, vitest + `@vscode/test-electron`. **Reuse the established patterns** — the formatting-toggle editor-mutation shape (Learning #21) and the math-preview webview shape (Learning #22: pure detection + pure HTML/CSP builder + thin adapter, vendored MIT asset, faithful tabGroups verification).
