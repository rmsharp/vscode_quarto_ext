...
## ACTIVE TASK
**Task:** **Session 21 DONE — Phase 6d Slice 6d-4 (front-matter top-level KEY completion)**. SHIPPED + adversarial-review-hardened. Inside the `---`…`---` front-matter block, typing a top-level key now offers document-key suggestions (`title`, `author`, `format`, `bibliography`, `toc`, `csl`, …) read from the user's **installed** Quarto schema (`schema/document-*.yml`, the 378 visible flat names), degrading to `CURATED_FRONTMATTER_KEYS` on any failure. Built on NEW model surface (`findFrontMatter`/`inFrontMatter`/`frontMatterContentLines`) and the existing 6d-3 reader/adapter. **Recommended next deliverable: implement Slice 6d-5** (front-matter top-level VALUE completion — enum/boolean values after a top-level `key:`; the value-side complement of 6d-4, reusing the 6d-2 value machinery) — but the operator may pick otherwise (a Phase 7 slice, 6e, a Polish item, the operator-only `vsce publish`, or 6d-6 nested). *Background: v1 feature-complete + release prep AGENT-COMPLETE (Sessions 1–13); 14 formatting toggles, 15 math preview, 16 diagram preview, 17 6d plan, 18 6d-1, 19 6d-2, 20 6d-3.*
**Status:** Shipped + committed (8 commits: the standalone Phase 1B stub + 5 layer-checkpoint feature commits [layers 4+5 share one] + 1 review-fix doc commit + this close-out docs commit). Baselines now **361 unit + 88 integration** green (was 326/81; +35 unit / +7 integration). Clean **33-file** `.vsix` (1.3 MB). `npm audit` unchanged = 7 dev-only vulns (Learning #20; **no new deps** — front matter reuses the 6d-3 reader). §3.3 guardrail intact (no `vscode` import in `core/`). Repo PUBLIC; `origin` → `rmsharp/vscode_quarto_ext` (branch `master`). At orientation `master` was **up to date with `origin`** (the operator pushed Sessions 17–20 since S20's notes); **this session's ~8 commits are UNPUSHED — confirm with `git status`; pushing is the operator's call.**
**Plan:** **6d is governed by `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md`** (per-phase plan). 6d-1 + 6d-2 + 6d-3 + 6d-4 are done; **6d-5** is next (plan §6 — front-matter top-level VALUE). Other v2 phases (Phase 7 remaining slices, 6e) are implemented directly from the parent plan. Strict TDD throughout.
**Priority:** Implement Phase 6d, one slice per session, **next is 6d-5** (plan §6). Each slice = strict TDD, §3.3 pure-`core/` guardrail, vitest + `@vscode/test-electron`.
**⚠ STRICT TDD IS MANDATORY** for any code/bugfix (operator directive — `CLAUDE.md` §"Mandatory development practice" + Learnings #10, #14, #15, #16, #21–#27, **#28**). Pure config/grammar/doc edits with no logic are exempt but still need their normal verification (compile, package, AND — per **Learning #18** — `npm run test:integration` after any `publisher`/`name`/activation change; the suites hard-code the extension ID).

### What You Must Do (recommended: implement Slice 6d-5; operator may redirect)
**Read `docs/planning/2026-06-28-phase-6d-yaml-completion-plan.md` §6 (Slice 6d-5) first.** 6d-5 = **front-matter top-level VALUE completion** — after a known top-level `key:` in the front matter, offer that key's enum/boolean values (`toc: true/false`, `number-sections: true/false`, …). It is the value-side complement of 6d-4 and reuses the 6d-2 cell-value machinery: add a `frontmatter-value` branch to `completionContextAt` (the front-matter line's value slot — mirror `topLevelKeySlot` for the value token PAST the colon; reuse the comment-termination idea from `slotsOf`), wire the provider's `frontmatter-value` kind to `index.frontMatterKeys(parentPath)` value lookup + the leading-space-on-`:` normalization (already in `valueItems` — generalize it), and add `values` to the relevant `CURATED_FRONTMATTER_KEYS` (grounded against the schema; **exact-equality tests, not `toContain` — Learning #26**). `parseSchemaIndex` ALREADY resolves `values` for document keys (it reuses `toField` → `valuesOfSchema`), so the reader path may already surface front-matter values — **verify against the real schema first** (many top-level doc keys are `boolean`/enum). **Inverted gating both directions** (no FM value items in prose/cells; `@` still suppressed in FM). **One slice, close out** (FM #18 — do NOT also do 6d-6 nested). If the operator instead wants a non-6d item: Phase 7 slices (snippets, image paste, graphviz `{dot}` rendering — Learning #23f), Phase 6e (🐉 offset-mapping), a Polish item (BACKLOG — spawn-timeout, cache-retry, the new partial-schema-degradation item), or the operator-only `vsce publish`.

### Useful starting context
- **★ Phase 6d-4 SHIPPED (Session 21, → Learning #28) — the 6d-5 executor reuses all of this:** `src/core/qmd/model.ts` `findFrontMatter(text)→{startLine,endLine}|null` (fence-inclusive span; a view over the single `scanRegions` pass — captured in the new `FrontMatterSpan{startLine,endLine,terminated}` field), `inFrontMatter(text,line)` (interior-content predicate, fence lines excluded; the value-side discriminator should reuse this gate), `frontMatterContentLines(text)→string[]|null` (interior lines; `core/frontmatter.ts` was FOLDED onto this — its duplicate `---` parser deleted). `src/core/yaml-context.ts` `completionContextAt` gained a `frontmatter-key` branch (`frontMatterKeyContextAt` + `topLevelKeySlot`, both private) returning `{kind:"frontmatter-key", parentPath:[], token, replaceRange}` (no engine); **6d-5 adds a `frontmatter-value` branch using the SAME front-matter-line value slot.** `src/core/yaml-schema.ts` `SchemaIndex.frontMatterKeys(parentPath)` (top-level when `parentPath==[]`, else `[]`); `parseSchemaIndex` collects `schema/document-*` via the shared `collectFields(data, prefix, defs)`; `CURATED_FRONTMATTER_KEYS` (13 keys, own descriptions). `src/providers/yaml.ts` `frontMatterKeyItem` (detail **`"Quarto document option"`**); the provider handles `frontmatter-key`. Tests: `test/unit/front-matter.test.ts` (14 — incl. the break-revert-proven agreement guard), `test/unit/yaml-context.test.ts` (now 26; +8 FM-key), `test/unit/yaml-schema.test.ts` (now 20; +4 curated FM), `test/unit/yaml-schema-index.test.ts` (now 30; +7 FM parse/degradation), `test/integration/suite/yaml.test.ts` (now 95 lines longer; +7 FM-key incl. the `csl` schema-only enrichment proof + both-direction gating). **⚠ The flat `schema/document-*.yml` names OMIT pure container keys like `execute`** (they live in `schema/schema.yml`'s object graph — recursive resolution, deferred to 6d-6); the 378 flat names ARE most top-level keys + hoisted leaf options (`format`, `title`, `csl`, …). `execute` is curated-only (a documented limitation).
