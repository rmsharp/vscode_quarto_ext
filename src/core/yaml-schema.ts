...
export interface SchemaField {
  ...
  description?: string;
  /**
   * The allowed values for value completion (enum / `[true,false]`). Reserved for
   * Slice 6d-2; unset here.
   */
  values?: string[];
  /**
 ...
 * the runtime schema reader (6d-3) is unavailable. Names verified to exist in the
 * Quarto 1.7.33 schema (`schema/cell-*.yml`); `fig-width`/`fig-height`/`cache`
 * are knitr-only per that schema's `tags.engine`.
 */
export const CURATED_CELL_OPTIONS: SchemaField[] = [
  { name: "echo", description: "Show the cell's source code in the rendered output." },
  {
    name: "eval",
