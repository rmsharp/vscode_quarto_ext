...
- **Model:** Claude Opus 5.
- **The defect.** `divColumns` held one column per open div and nothing tied that column to the container the div lived in, and the stack was never truncated when a container ended. So a stale column outlived its container and a `:::` in a SIBLING or DEEPER container matched it. Closes the item Session 227 filed from its own Round 5 and Session 228 ranked #1.
- **⚠ THE PREMISE WAS CHECKED FIRST, AND UNLIKE SESSION 228'S IT HELD.** S228's gotcha 1 says a filed item is a hypothesis: re-run its own cited witnesses against the filing session's build. `git archive 3717e624` into `presrc/`, scored against HEAD — `src/` is byte-identical between the two, and both report the phantom on both witnesses (`s227/r5/q03` → `["H q03"]`, `r5/q06` → `["H q06"]`, quarto renders `[]` for each). Two minutes, and the session proceeded on measured ground.
- **⚠ THE FINDING: ONE FILED ITEM WAS TWO RULES, AND ONLY ONE OF THEM IS ABOUT IDENTITY.** The item's own prescription — a container identity on the stack — fixes the *stale* half. It does not fix the *deeper closer* half, and the natural implementation of it (compare container depths) **deletes a real heading on the first guard row**: rendered, `::: {.note}` / `- item` / `:::` closes the top-level div and renders the heading, yet the list container is still open at that closer's line so the depths differ by one. What separates the two halves is the COLUMN — lazy absorption carries a shallow line DOWN into a container's open paragraph and has no upward direction. (Learning #407.)
- **⚠ AND THE DELETION THE SESSION ACTUALLY INTRODUCED WAS FOUND BY THE 13-DOCUMENT ROUND, NOT BY THE 47,871-DOCUMENT SWEEP.** Entering a block quote CLEARS `contentColumns` — the quote's content base is its own column 0, the outer stack is stashed in `outerColumns` — and read as a depth that clear is indistinguishable from every container ending at once, so the new auto-close closed a div whose list item was still wide open (`r2/b01`, rendered: the `<div class="note">` is complete inside the `<li>`). ⚠ It cannot be recovered from the indent, and this was **instrumented rather than reasoned**: by the time the auto-close runs the line is the STRIPPED one and `indentWidth` is 0 both for `b01`, whose item survives, and for `r2/b13`, whose item really does end. So each entry carries its quote context and the auto-close is SUSPENDED across a switch — the boundary `quoteColumnsUnknown` already draws, for the same reason: keep the phantom, never risk the deletion. (Learnings #408, #409.)
- **The rule, measured rather than ported.** Each `divColumns` entry now carries `containerDepth` (`contentColumns.length` at its opener) and `inQuote`. A div is auto-closed when the container-pop loop ends the container that hosts it — deliberately inside that branch and above the push, so a sibling container opened on the same line cannot resurrect the div the pop just orphaned. And the lazy branch of the closer rule gains its one direction: `divFence.column <= divColumns[top].column`.
- **TDD — the guard block FIRST, then RED→GREEN.** S204's gotcha 5 inherited a twenty-third time. **14 `it()`s green BEFORE the change**, including the two rows a wrong fix breaks in opposite directions (`r1/a04`, `r1/a16`). **3 RED→GREEN cycles**, each RED confirmed to fail for the right reason. **3 residual pins**, each against its own rendered witness and each scored pre AND post.
- **Measurement.** **34 designed documents across two rounds**, rendered through the real `quarto render` path (quarto 1.7.33), predictions frozen and SHA-256'd before every render — **33/34 predicted** (21/21, 12/13; the one miss is quarto's own synthetic `Footnotes` appendix heading, an instrument artifact, not a model fact). Model vs quarto, scored row by row pre and post: **r1 14 → 21**, **r2 8 → 9**, **8 recovered, 0 introduced**. Repo sweep: **113 tracked documents, 0 movers.** Corpus sweep: **47,871 documents, 13 mover rows, all headings, 0 labels, 0 cells** — one inherited predecessor document (`s183/R3-fenceddiv-refute/run2/C08_list_in_div_close`), re-rendered and confirmed a RECOVERY against both its own recorded render and a fresh one; the rest are this session's own rows and the two filed witnesses, every one scored against a rendered document. **ZERO DELETIONS.** The sweep was re-run to fixpoint after the cycle-3 fix and the mover set was identical, with no new movers.
- **⚠ Three residuals pinned and filed, each PRE-EXISTING and unmoved (scored pre and post).** `r2/b09` — ⚠ the one in the DELETION direction: two divs at one column, the outer closed lazily, and this model renders nothing where quarto renders the heading. `r2/b11` — a div opened inside a quote and closed outside it. `r2/b13` — the phantom the quote suspension deliberately buys back.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2335 passed / 66 files** (baseline 2315, **+20**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S228**) · `check-package` OK 42 files / 5.57 MB · `check-backlog` OK, 165 open items · `test:integration` **542 passing / exit 0**, on a go-ahead sought in advance, assertions pre-checked headlessly first. NOT RUN: `test:lsp` — no LSP surface touched.

### 2026-08-19 · [ad hoc] Session 228 — the setext one-line boundary: the filed item was REFUTED, and the boundary is now pinned

- **Model:** Claude Opus 5.
- **The item, and why no `src/` line changed.** Session 226 filed *"`---` under a ONE-line paragraph is a setext underline; under a TWO-line paragraph it is an EM DASH"*, phantom direction, *"this model reports the heading BELOW the dashes that quarto does not"*, and Session 227 ranked it #1. Re-run against the build of the session that filed it (`git archive 911ed697`) and against HEAD, **this model answers both cited witnesses exactly as quarto renders them**: `s226/r3/h03` → `["para one"]`, `s226/r1/t_tbreak` → `[]`. The miss was in Session 226's own `scratchpad/s226/r3/PREDICTIONS.json`, which records `h03: []` against a rendered `["para one"]` — the SESSION's frozen prediction, not the model's output. (Learning #404.)
- **⚠ The item's second claim was checkable in one grep and also wrong.** It said the separating rule was *"unmodelled rather than approximate"*. The rule is `consecutiveBody === 1`, in `SETEXT_H1`'s docstring since Session 181, citing the same `pandoc -f markdown` measurement the item re-derived.
- **What the session actually shipped: the half-pinned boundary.** The 2+-line rule was pinned in the `=` spelling alone (`["Line one", "Line two", "========"]`); the `---` spelling the item named — the one whose dashes become U+2014 — was pinned nowhere, so the rule the item accused could have regressed silently. Six pins now cover it: the boundary itself at 1/2/3 lines, run length (`-`, `--`, `----`), the underline's indent as an EQUALITY rather than CommonMark's 0-3 window, what may be a TITLE, the `paragraphOpen` bail on `opensFreshBlock`, and the two cited witnesses at the answer quarto renders.
- **⚠ Green is not evidence — every pin was mutation-checked.** `consecutiveBody === 1` → `>= 1` kills P1/P2/P5/P6; the column equality → a `0-3` window kills P3; barring a thematic-break-shaped or indented line from being a title kills P4/P5. No pin survives every mutant, so none is testing the corpus instead of the code. `src/core/qmd/model.ts` was restored byte-identical (SHA-256 verified) after each mutant. (Learning #405.)
- **Measurement.** **46 designed documents across three rounds**, rendered through the real `quarto render` path (quarto 1.7.33), predictions frozen and SHA-256'd before every render. **Model vs quarto: 43/46, identical pre and post** (`recovered 0, introduced 0`). **My own predictions vs quarto: 33/46** — and on every row where the two disagreed the model was right and I was wrong. All three model misses are pre-existing and none belongs to this rule: `r1/a15` is Session 225's already-filed quoted-setext residual, `r3/c03` the deliberate empty-heading omission, `r3/c11` this model's file-wide convention of carrying literal source text (`# *em*` reports `*em*` too — measured). (Learning #406.)
- **Three facts the rounds established that nothing recorded before.** An `=` run that no underline consumed is claimed as a TITLE by the `---` below it (`<h2>===</h2>`); a `***` — a thematic break by every other measure — is claimed as a title too; and a 4-space INDENTED line, indented code anywhere else, is claimed as well. Also: `para one` / `---` / `para two` / `---` does not render at all — quarto reads the two `---` lines as a mid-document metadata block and exits 1 with a YAML error, which is a corpus-design trap rather than a model defect.
- **TDD.** No RED→GREEN cycle: there was no defect to fix, and the gate's own scope is *"anything with logic"*. The analogue applied instead is the mutation check above — each pin shown to FAIL against a mutant expressing its axis before being trusted.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `npm test` **2315 passed / 66 files** (baseline 2309, **+6**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S227**) · `check-package` OK 42 files / 5.57 MB · `check-backlog` OK, 163 open items. NOT RUN: `test:integration` and `test:lsp` — `src/` is byte-identical to HEAD, so no runtime surface changed (Phase 3E stated, not skipped).

### 2026-08-19 · [ad hoc] Session 227 — a `:::` is a fence only at its container's own column (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** `DIV_FENCE` was anchored at `^ {0,3}` — CommonMark's leading-space tolerance for a CODE fence, applied to a pandoc DIV, which has none. Pandoc's `divFenceEnd` reads the colon run with no leading-space parser at all, so the run must begin exactly where the enclosing block's content does — the same absolute equality `setextUnderlineLevel` and `atxHeadingMatch` already apply. Closes the item Session 226 filed and ranked #1, its own Round 4 residual.
- **⚠ THE FINDING: ONE WRONG ANCHOR WAS TWO OPPOSITE DEFECTS, AND THE FILED ITEM DESCRIBED ONLY THE SMALLER ONE.** It ACCEPTED a `:::` one to three columns off at the top level — the phantom the item named — and it also accepted an indented OPENER, which the item did not mention (rendered, `  ::: {.note}` emits `<p>::: {.note}</p>`, no div at all, at every indent 1–3). And it REFUSED every fence in a container deeper than column 3: a div inside a two-deep list was never opened, its closer closed nothing, and the paragraph ran across a real heading. The 47,711-document sweep split its 74 movers into 65 removals and **9 ADDITIONS**, and the additions are headings this model had never once been able to see. (Learning #401.)
- **The rule, measured rather than ported.** A `:::` line is a div fence — opener or closer — only where its colon run starts at a column in `[0, ...contentColumns]`. Not a bare equality, because pandoc absorbs an under-indented line into the container's open paragraph: a column-0 closer really does close a column-2 div (`r1/k17`), and a three-deep one closes at the OUTER item's column 2 (`r2/m02`), while column 1 belongs to no container and closes nothing at either depth (`r1/k18`, `r2/m11`). Absorption needs an open paragraph to absorb into, so under a blank line the only column that closes is the div's own — hence a stack of columns rather than a counter (`r1/k25`, `r2/m04`). A run at CODE DEPTH is dropped before its role is read, so a four-space `:::` at the top level stays the indented code block it has always been and the heading after it survives (`r1/k10`).
- **⚠ THE 8 DELETIONS THE SWEEP FOUND, AND NOTHING ELSE COULD HAVE.** A list item and a block quote do not absorb a lazy line the same way: the item appends it RAW, the quote STRIPS its 0–3 leading spaces. So an unmarked ` :::` below a quote lands on the quote's own column 0 and really does close the div opened inside it — re-rendered, the heading is a real `<h1>` inside the `<blockquote>` in all eight rows. My designed corpus was built from top-level and list-item shapes, had no shape for this at all, and scored 25/25 and 13/14 while the build was deleting every one. The fix is the suspension `quoteColumnsUnknown` already draws for the raw-TeX row: where a column cannot be computed, keep the pre-session width. (Learning #402.)
- **⚠ Round 5 was designed against the IMPLEMENTATION rather than the rule**, and is the only corpus this session's author was not blind to: it asked what a stack of columns with no container identity would get wrong, scored **5/5** on frozen predictions, and turned two suspected weaknesses into filed, pinned residuals with rendered witnesses — both PRE-EXISTING and unmoved, verified by scoring each row pre and post. (Learning #403.)
- **Measurement.** **56 designed documents across five rounds**, rendered through the real `quarto render` path (quarto 1.7.33), predictions frozen and SHA-256'd before every render — **48/56 predicted** (21/25, 12/14, 7/8, 3/4, 5/5), and every miss became a finding. Designed corpus scored row by row, pre vs post: **30 → 51 of 56**, 22 recovered, 1 introduced. Repo sweep: **113 tracked documents, 0 movers.** Corpus sweep: **74 movers across 47,711 documents**, every one rendered and scored — **9 correct before, 71 after, 63 RECOVERED, ZERO DELETIONS**; the sweep was re-run to fixpoint after the C5 fix and the new mover set was a strict SUBSET, with no new movers. **The one introduced row is `r2/m07`, a document this session wrote**: 42 inherited predecessor documents moved and every one is correct after.
- **TDD.** 19 guard `it()`s green BEFORE the change (`9071a6ba`), one per shape whose heading must survive plus the phantom rows that must not reappear. **5 RED→GREEN cycles**, each RED confirmed to fail for the right reason: `26974d31` the closer's column, `e1108ece` the stack and the lazy precondition, `a23e608d` the opener, `b86369e8` the widening plus the code-depth guard, `0ba88092` the quote suspension. **2 residual pins** (`3a58f167` and R1), each against its own rendered witness.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2309 passed / 66 files** (baseline 2283, **+26**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S226**) · `check-package` OK 42 files / 5.57 MB · `check-backlog` OK, 164 open items · `test:integration` **542 passing / 0 failing / exit 0**, green first time on a go-ahead sought in advance, both div-touching tests watched BY NAME at lines 336 and 337 of `scratchpad/s227/integration.log`, assertions pre-checked headlessly first. NOT RUN: `test:lsp` — no LSP surface touched.

### 2026-08-19 · [ad hoc] Session 226 — a closer that closes nothing is not a closer (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** `CLOSER_LINE` and `RAW_TEX_ENV_OPEN` are tested AHEAD of `closesParagraph`'s `paragraphOpen` bail, because a closer follows its own construct's content and so, to a per-line scanner, always looks like it sits against an open paragraph. That is right for a closer that really closes something and wrong for one that does not — and nothing on the line separates the two. Closes the item Session 225 filed and ranked #1, its own R1/R2 residual pins.
- **⚠ THE FINDING: THE ITEM WAS FILED UNDER A PREMISE THE RENDERS REFUTE.** It was scoped *"inside a quote"* and sized at 24 documents in 46,556, with the note *"both need the quote's own BLOCK state — scope as the next slice of the container capability."* Rendered BOTH ways, quarto's answer inside a block quote is **identical** to its answer at top level on **every one of the 13 shapes**, and this model was wrong on the same three constructs in both places. The construct decides it, not the container; the real size was **405 movers**, ~17× the filed estimate. Operator was consulted on the scope fork and chose to measure both gates and let the data decide; the data left no quote gate to add. (Learning #398.)
- **The rule, measured rather than ported.** A `:::` is an opener when it carries a tail and no paragraph is open, a closer only at depth > 0, and otherwise ordinary paragraph text — which, with none open, STARTS a paragraph, so `closesParagraph` returns `false` rather than falling through. A `...` terminates only a metadata block that is actually open. A `\begin{}` is a block only when its matching `\end{}` exists at or below its own line; an `\end{}` closes only the environment of its own NAME. A line block takes a continuation only from its own container, which is the lazy-continuation half.
- **⚠ THE DESIGNED CORPUS SCORED 78/78 WITH INTRODUCED 0 WHILE THE CHANGE WAS DELETING 39 REAL HEADINGS.** Four rounds, 78 documents, predictions frozen and SHA-256'd before every render (27/29, 23/28, 15/16, 4/5 — and every miss became a finding). The 47,125-document sweep plus a render of EVERY mover found three families the corpus had no shape for: a one-line `\begin{x}…\end{x}` (19), a div opener sharing its line with a LIST MARKER (18), and an `\end{}` matched by depth rather than by name, so a lookalike inside a `verbatim` block consumed it (2). A fourth, `\end{b}\end{a}`, was found by re-reading the diff and then rendering the row. Each was adjudicated by re-rendering, never by reasoning. (Learnings #399, #400.)
- **Measurement.** Designed corpus **47 → 77 of 78**, INTRODUCED 0. Repo sweep: **1 mover across 113 tracked files — a RECOVERY**, verified against its own render (`quarto render` does not emit `Below A Div Closer`, so the fixture's own heading text was named for an assumption the render refutes; a Session 183 integration assertion pinning it as a deliberate residual is REVERSED). Corpus sweep: **405 movers across 47,125 documents, every one exactly ONE heading removed and zero added.** All 454 documents that moved at any point were rendered and scored: **39 correct before, 397 after, 358 RECOVERED, INTRODUCED 0**; the 2 movers outside that set were rendered separately and are both recoveries. **ZERO DELETIONS at ship.**
- **Three predecessor pins REVERSED**, each against its own re-rendered witness: Session 225's R1 and R2 residuals, and the Session 183 outline integration assertion. 16 guard `it()`s green BEFORE the change — and ⚠ **the guard refuted a shortcut within the minute**: inferring "a `...` that reaches this line terminates nothing" from where the code sits deleted two real headings, because `consumedMetadataLines` reads RAW lines and recognises only a blank-preceded span.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2283 passed / 66 files** (baseline 2257, **+26**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S225**) · `check-package` OK 42 files / 5.57 MB · `check-backlog` OK, 163 open items · `test:integration` **542 passing / 0 failing / exit 0**, green first time on a go-ahead sought in advance, the reversed test watched BY NAME at line 338 of `scratchpad/s226/integration.log`, assertions pre-checked headlessly first. NOT RUN: `test:lsp` — no LSP surface touched.

### 2026-08-19 · [ad hoc] Session 225 — inside a block quote, the constructs are seen at last (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** This model had no block-quote context anywhere: `FENCE_OPEN` is anchored at `^[ \t]*` and `findHeadings` never matched a `> # ` line, so a heading, a fence, or the label on a fence inside a quote was invisible — and, in the other direction, the contents of a quoted code block leaked out as prose and minted phantom labels. Closes the item Session 224 filed and ranked #1, the one row of its 72-row scorer still marked wrong.
- **The rule, measured rather than ported.** Quarto strips a quote's markers and re-parses what is left. The marker is `^ {0,3}(>[ ]?)+` — one space, and ⚠ **a TAB is not it**; the quote's content base is column 0 **exactly**, the same absolute equality quarto applies at top level; a quote may not interrupt an open paragraph; an unmarked BLANK line ends the quote while an unmarked non-blank line does not; and a fence inside a quote is closed by a marked **or** a lazy closer, but only before the quote ends.
- **⚠ The finding the filed item did not have: quarto's two LINE-ANCHORED intercepts do not reach inside a quote, so the info string faces pandoc's `Attr` parser alone.** Rendered both ways from identical bytes — ```` ```{ojs} ```` is a real cell at top level and an inline code span in a quote; ```` ```{#lst-x bad} ```` is `<pre class="…">` at top level and not a fence at all in a quote, where the `# Heading` between the backticks is a real `<h1>`; and ```` ```{#lst-x} ```` defines **no** id at top level and a **real** one in a quote. Reusing the top-level predicate would have minted a cell quarto never executes and deleted a heading it really renders.
- **⚠ Every defect after the first GREEN came from state the stripped line left for the lines BELOW it, and the 46,556-document sweep is what found all four.** Clearing the container-column stack on entering a quote deleted four families of real headings (a quote may sit inside a list item whose content column still governs everything below it); Session 183's `CLOSES_PARAGRAPH` suspension made three families of quoted openers close a paragraph pandoc keeps open; a stripped line could open a CommonMark HTML block that outlived its quote; and a setext title was read raw, marker and all. Each was adjudicated by re-rendering the row, never by reasoning.
- **Three disclosed bounds, each free in the safe direction.** The model reported nothing inside a quote before this session, so a decline inside one returns the pre-session answer exactly. The setext path is declined on a stripped line (quarto consumes a quoted three-dash/key/three-dash triple as metadata exactly as it consumes a top-level one — an empty `<blockquote>`); a CommonMark HTML block is not opened from a stripped line; and a setext title part-way into a quote under an unmarked underline is refused. All three are filed.
- **Measurement.** 45 designed rows rendered through the real `quarto render` path (quarto 1.7.33) across five rounds, predictions frozen and hashed before each — round 2 scored 11/13 and round 3 7/9, and **both of the rules I got wrong became headlines**. Designed corpus scored row by row, pre vs post: **27/45 → 40/45, INTRODUCED 0**. Sweeps: **1 mover across 113 tracked repo files** — two real `> ### ` headings recovered in `docs/planning/`, verified against their own render — and 385 mover rows across **46,556 corpus documents**, every distinct mover document (375) re-rendered and scored: **25 correct before, 296 after, 295 recovered, 24 introduced**. ⚠ **Zero real deletions**: all 16 removal rows are phantom removals, each verified against its own render.
- **⚠ The 24 introduced are one shape and are pinned in both directions (R1, R2) and filed** — a block construct inside a quote (`:::`, a grid border, a pipe row, `\begin{}`, a line block) whose block-ness this scanner decides from the line alone, and an unmarked line below a quote, which pandoc reads as a lazy continuation this model does not model.
- **Six predecessor pins REVERSED**, each against its own re-rendered witness: Session 193's Foxtrot row, Session 214's setext swallow, Session 215's tight hash, Session 217's escape, Session 218's attribute block (now carrying its `sec-` id) and Session 224's P10. One inherited test **refuted a bound within the minute** — a quoted title under an *unmarked* underline keeps its marker, and quarto renders it that way.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2257 passed / 66 files** (baseline 2236, **+21**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S224**) · `check-package` OK 42 files / 5.56 MB · `check-backlog` OK, 160 open items · `test:integration` **542 passing / 0 failing / exit 0** (baseline 540, +2), green first time on a go-ahead sought in advance, both new tests watched BY NAME at lines 96 and 366 of `scratchpad/s225/integration.log`. NOT RUN: `test:lsp` — no LSP surface touched.


### 2026-08-19 · [ad hoc] Session 224 — a fence opener whose info string quarto refuses opens no code region (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** When pandoc's `[word] {attrs}` parse of a fenced code block's info string fails, quarto does not fall back to a plain code block — **the fence is not a fence**: its opener renders as ordinary text and its contents are live prose. `scanRegions` opened a region anyway, so a heading, an `@ref` or a `{#fig-…}` inside such a block was invisible to this model. Quarto's own crossref filter is the proof: rendering the survey corpus emitted `Unable to resolve crossref @fig-s03`, a use this model could not see at all. Closes the item Session 223 filed and ranked #1.
- **⚠ The finding: the filed item described half of it, because a refused delimiter RE-PAIRS.** A rejected opener's CLOSING fence becomes an OPENER in quarto, so the block *after* it is code to quarto and prose to us — `scratchpad/s224/cal/sv.qmd` s03's closer swallows the whole of s04, its comment line and opener included. One document therefore shows both a lost-TP and a phantom direction from the same defect.
- **⚠ And the item's own mechanism sentence contained a contradiction that made the obvious implementation wrong.** It said the backticks "read as an INLINE CODE SPAN **and** the block's contents are prose" — content inside a code span is literal, not prose. The missing variable is the BLANK LINE: a code span cannot cross one. With no blank line the opener and closer form one span, the contents are literal, and hiding them as a region is accidentally right; with a blank line the backticks are literal text and everything below is live. Implementing the item as written lost a true positive (`s223/cal/disc.qmd` d11) and minted a phantom — both caught by the pre/post scorer run after the first GREEN, before either reached a commit.
- **The rule, measured rather than ported.** At most one bare word, holding no `{` and no `}`; at most one word before a block that ENDS the info string; quarto's own stage 1 (a brace-led string with neither `.` nor `=`) is a FENCE with a literal class; a RAW attribute `{=html}` is a fence and is **not** a valid `Attr`; otherwise the block must be a valid attribute block. ⚠ **The whole refusal is PANDOC-FAMILY only** — under `commonmark_x` all nine otherwise-refused rows build a code block (round 3, 9/9), so a reader-blind predicate would have deleted CommonMark regions wholesale.
- **⚠ The `{=html}` row is the one that would have shipped a wide, silent regression.** A predicate built on attribute validity alone refuses every raw block in every real document; no unit test in the repo covered it. It was caught because the round-4 prediction sheet marked it uncertain and rendered it.
- **Measurement.** 72 rows rendered through the real `quarto render` path (quarto 1.7.33) across seven rounds, predictions frozen and **hashed** before each: survey 9/10, b 12/14, c 9/9, d 12/13, e 8/8, g 4/5, adversarial 10/11. Ground truth extracted MECHANICALLY from the rendered HTML. Scored row by row, pre vs post: **36/72 → 71/72, INTRODUCED 0**. The one remaining miss is the block-quote residual, wrong the same way before and after, now filed. Sweeps: **0 movers across 113 tracked repo files**; 25 mover rows across **46,598 corpus documents**, every one run down individually — 19 in this session's own corpora and **6 predecessor recoveries**, each verified against its own render (four phantom headings in Session 183's corpus that quarto does not render, and the two phantom labels Session 223 pinned at H8/H12).
- **⚠ Two disclosed consequences.** Source 3's unvalidated narrow scan had to be withheld on any line beginning with a fence run, because every fence newly refused becomes such a line and would otherwise mint a phantom id — that CLOSES two rows Session 223 pinned as belonging to a wider filed item (H8, H12, both REVERSED with their rendered witnesses; the item stays open for the shapes with no fence run). And `braceGroups` moved down from `refs.ts` to `model.ts` so the region scanner and the label scanner share one grammar rather than two (Learning #14) — a pure move, committed on its own with no behaviour change.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2236 passed / 66 files** (baseline 2208, **+28**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S223**) · `check-package` OK 42 files / 5.56 MB · `check-backlog` OK, 160 open items · `test:integration` **540 passing / 0 failing / exit 0** (baseline 538, +2), green first time on a go-ahead sought in advance, both new tests watched BY NAME at lines 95 and 364 of `scratchpad/s224/integration.log`. NOT RUN: `test:lsp` — no LSP surface touched.
- **TDD.** Guard block of **14 `it()`s** written and run GREEN before any production change, on this change's own polarity — a NARROWING of the region set, so every row is a fence that must keep hiding its contents. **4 RED→GREEN cycles**, each RED confirmed to fail for the right reason; one of them was an inherited Session 223 test going red, and one was driven by this session's own adversarial pass rather than a designed row. **10 further behaviours declared PINS rather than counted as cycles**, one of them a disclosed residual rather than a pass. **2 predecessor pins REVERSED.**
- **Files.** `src/core/qmd/model.ts` (`fenceInfoOpensBlock`, `RAW_ATTRIBUTE`, `consumedCloserLine`, `braceGroups` moved in), `src/core/refs.ts` (the fence-run clause in Source 3), `test/unit/qmd-model.test.ts`, `test/unit/refs.test.ts`, `test/integration/suite/outline.test.ts`, `test/integration/suite/crossref.test.ts`.

### 2026-08-19 · [ad hoc] Session 223 — a fenced code block's attribute block defines its cross-reference id (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** `lst-` is one of the five kinds `RefKind` claims to support and it was the only one with **no working definition source at all**. ```` ```{#lst-p06 .python} ```` renders `id="lst-p06"` in quarto and this model indexed nothing, because a fence opener is a region BOUNDARY: `findBodyLines` never yields it and Sources 1–3 never saw it. Closes the item Session 222 filed and ranked #1, and reverses that session's own G5 pin, which asserted the defect with its own rendered witness.
- **⚠ The finding: this production has a stage pandoc does not have, so porting `Attr` would have minted a phantom on the commonest shape of all.** ```` ```{#lst-s03} ```` — the attribute block holding nothing but the id — defines **no id at all** under quarto (`<pre class="{#lst-s03}">`, braces kept as a literal class), while `quarto pandoc -f markdown -t html` defines `id="lst-s03"` from the identical bytes; so do `commonmark`, `commonmark_x`, `gfm`, `+attributes`, `+raw_attribute` and three further flavours. **None of the eight reproduces quarto's split**, and `quarto render --to native` shows the class already in the AST. The measured gate: an info string that BEGINS with `{` and contains neither `.` nor `=` is intercepted. ⚠ It is LEXICAL, not a validity test — `{#lst-t01.b}` is released by a `.` inside the IDENTIFIER and defines `lst-t01.b` with no class at all, while `{#lst-t02:b}` is intercepted, and 23 rows holding `: , _ - é $ ! * / + ; % | ( ) ~`, a space, a bare word or a second `#id` atom are intercepted too. ⚠ **The mechanism inside quarto was NOT identified** — bounded to "not any pandoc reader flavour tried, and present before filters run", and reported as unidentified rather than asserted.
- **The rule, measured rather than ported.** Stage 1 is quarto's gate above; stage 2 is pandoc's `Attr`, which `attributeBlockId` already carries measured — so the reader split (pandoc-family takes the LAST atom, `commonmark_x` the FIRST) came free rather than being declared away, which is the half Session 222 had to fix after the fact. The block must END the info string and may be preceded by at most ONE info-string word; a failed stage-2 parse means the fence is not a fence at all.
- **⚠ Two phantoms this session introduced and its own scorer caught, one of them inside a commit.** Cycle 1 passed its RED, kept all 12 guard rows and 2208 unit tests green, and minted an id from ```` ```{#lst-d10 .python} extra ````, which quarto does not read as a code block at all — a row already measured in round 2 before the cycle was written. Cycle 3's move to the LAST brace group (measured, correct) introduced a second on `{#lst-y03a .cls}{#lst-y03b}`. Both were caught as `INTRODUCED 1` by the row-by-row scorer run after each GREEN, and both are closed in `43121969`.
- **A defect found by the guard block and deliberately NOT closed.** Two rows were RED when the guard was written: an UNTERMINATED fence line and a top-level INDENTED one are ordinary body text — quarto renders both verbatim and defines nothing — and Source 3's unvalidated scan already mints a label from each. That is the already-filed "a group with nothing in front of it" item seen on a fence line, so it is pinned at H8/H12 and the filed item updated, not folded in.
- **Measurement.** 51 rows rendered through the real `quarto render` path (quarto 1.7.33) across six rounds, predictions frozen and **hashed** before each: 12/13, 11/13, 12/13, 6/8, 8/10 (scored against a predicate the round itself refuted) and 10/10 confirming the survivor. Ground truth extracted MECHANICALLY from the rendered HTML rather than transcribed. Scored row-by-row pre vs post, split by failure kind, over 84 rows: **42/84 → 82/84, lost TP 40 → 0, phantom 2 → 2, INTRODUCED 0** — both surviving phantoms pre-existing and pinned. Adversarial corpus aimed at this implementation's own geometry: **3/14 → 13/14 → 14/14** across the two cycles it drove. Pre-vs-post sweep over **46,553 documents**: 16 movers, all additions, **every one run down individually** — 15 are this session's own rendered corpora, and the single predecessor mover is `s222/cal/p.qmd` gaining `lst-p06` at line 19 column 5, which is the filed item's own witness, verified against that session's rendered HTML. **0 movers across 113 tracked repo files.**
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2208 passed / 66 files** (baseline 2182, **+26**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S222**) · `check-package` OK 42 files / 5.56 MB · `check-backlog` OK, 160 open items · `test:integration` **538 passing / 0 failing / exit 0** (baseline 536, +2), green first time on a go-ahead sought in advance, both new tests watched BY NAME at lines 93–94 of `scratchpad/s223/integration.log`. NOT RUN: `test:lsp` — no LSP surface touched.
- **TDD.** Guard block of **12 `it()`s** written and run GREEN before any production change, on this change's own polarity — a WIDENING, so the guard is per source-that-must-not-move and per shape-that-must-NOT-start-defining. **4 RED→GREEN cycles**, each RED confirmed to fail for the right reason, two of them driven by rows the adversarial pass found rather than rows designed to confirm. **10 further behaviours declared PINS rather than counted as cycles**, because each was already green when written — including the reader split, which is the load-bearing one.
- **Files.** `src/core/qmd/model.ts` (`CodeFenceOpener`, `findCodeFenceOpeners`, recorded at the closer symmetric with the cell emit), `src/core/refs.ts` (Source 4, `fenceAttributeId`, `FENCE_RUN`), `test/unit/refs.test.ts`, `test/integration/suite/crossref.test.ts`.

### 2026-08-18 · [ad hoc] Session 222 — a brace group defines an id only when it is the element's attribute block (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect, both directions at once.** Source 3 scanned prose lines for the literal two characters `{#` and never asked whether the brace group was an attribute block. So an id that was not FIRST in its block was indexed as **nothing** — `![Cap](a.png){.cls #fig-a.x}` renders `id="fig-a.x"` and `@fig-a.x` resolves to it — while five measured group shapes quarto REFUSES each minted a label: `{#fig-a$b}`, `{#fig-a b}`, `{#fig-a#b}` (quarto defines `b`, so no `fig-` target exists), an unclosed `{#fig-a.x`, and eighteen further character-class rows in Session 221's own corpus. Closes two folded `BACKLOG.md` items, both filed by Session 221.
- **The rule, measured rather than ported.** A group contributes an id when it opens IMMEDIATELY after the `)`/`]` of an image, link or bracketed span, or is the whole tail of a `:::` div opener; its tokens are then validated and the id comes from the block's ATOMS. ⚠ **Round 2 refuted two rules that both explained a 33/34 round 1** — "the first group on the line wins" and "the first VALID group wins" — with `x {#fig-w01a} ![Cap](a.png){#fig-w01b}` (defines the second, renders the first as text) and `![A](a.png){#a} and ![B](b.png){#b}` (defines BOTH).
- **⚠ The finding: one surface, three productions, three grammars — and the filed item's "port the heading shape here" would have DELETED working targets.** Round 3 scored **4/10** and found that display math is not the attribute parser at all: `{#eq-x .cls}`, `{#eq-a #eq-b}`, `{#eq-x key=v}` and even `{#eq-x }` render as literal TEXT, while `{#eq-m06$x}` defines `id="eq-m06$x"` **verbatim** — a `$` the attribute parser categorically refuses. Table captions are a third machine again: a trailing class is fine, a leading one is not, and `{#tbl-n05$x}` is SANITISED to `tbl-n05x`. So the older scan is kept, deliberately, for the two productions quarto owns itself.
- **⚠ A regression this session shipped and its own sweep caught.** The reader split Session 219 measured on headings holds for inline blocks too (`commonmark_x` takes the FIRST atom). Commit `af058cb7` implemented the pandoc half alone and declared the other a bounded divergence — and the 46,530-document sweep showed the OLD scan took the first `{#…}` on the line, which is *accidentally correct* for that reader. `f492832c` threads an `AttributeBlockReader` resolving both flags from the same front-matter `from:` line `buildHeading` uses. Post-fix, `cmx.qmd`'s two previously-correct ids are unchanged in id **and** column.
- **⚠ An over-fire left standing because its fix is measured to be worse.** A top-level indented `  ::: {#fig-x}` is a phantom, and anchoring the regex at `^` would close it — but an indented div INSIDE A LIST ITEM really does define, and that is an everyday quarto shape. Pinned with the row that condemns the fix.
- **Measurement.** 38 single-line rows scored against the rendered ground truth: **12/38 → 29/38, lost-TP 3 → 0, INTRODUCED 0**. 73 rows rendered in total across five rounds through the real `quarto render` path (quarto 1.7.33), predictions frozen and **hashed** before each round: 33/34, 11/12, 4/10 (the refutation), 6/8, 6/7 + 3 `commonmark_x`, then 5/5 verifying the shared tokenizer. Adversarial corpus aimed at this implementation's own geometry: **7/9 → 7/9, FIXED 0, INTRODUCED 0**, the same two rows wrong before and after. Pre-vs-post sweep over **46,530 documents**: 8 movers, every one accounted for — 18 phantom `fig-` labels removed from `s221/cal/attr.qmd` (each confirmed against the rendered HTML to define no id), 1 from an indented code block in `CALIBRATION.md`, 1 phantom removed and 1 lost TP recovered in `s221/adv/adv.qmd`, and 5 of this session's own corpora. **0 movers across 115 tracked repo files.**
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2182 passed / 66 files** (baseline 2163, **+19**) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (**byte-identical to S180–S221**) · `check-package` OK 42 files / 5.55 MB · `check-backlog` OK, 159 open items · `test:integration` **536 passing / 0 failing / exit 0** (baseline 534, +2), green first time on a go-ahead sought in advance, both new tests watched BY NAME at lines 91–92 of `scratchpad/s222/integration.log`. NOT RUN: `test:lsp` — no LSP surface touched.
- **TDD.** Guard block of **13 `it()`s** written and run GREEN before any production change, on this change's own polarity — a NARROWING, so the guard is per shape-that-must-keep-defining and per source-that-must-not-move, including two explicit SCOPE pins for rows quarto gets right that this session deliberately did not close. **3 RED→GREEN cycles**, each RED confirmed to fail for the right reason. ⚠ A fourth was written and is **disclosed as demoted**: cycle 2's own `{#fig-a#b}` row is a valid block that can only yield nothing through atom SELECTION, and once atoms are read instead of the first two characters, position-independence is the absence of a clause rather than a further one. Two existing pins REVERSED against the measurement (A1, A2) and one residual pin rewritten (C1).
- **Filed, not fixed.** Six items: the `lst-` code-block production that has no working definition source at all (lost-TP, newly measured), the display-math/caption phantoms, the `)`/`]`-that-closes-nothing rule, the bare-prose group, the indented-div over-fire, and the never-asked `commonmark_x` VALIDITY half.
- **Commits.** `a024f4f3` (1B claim, nine decision rules from a coupling survey run before the stub) · `18d0b5a8` (guard) · `af058cb7` (the block-aware scan) · `f492832c` (the reader split) · `268e25f9` (adversarial pins) · `61b9fc7a` (integration).

### 2026-08-18 · [ad hoc] Session 221 — the definition side reaches the id quarto actually defines (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** `src/core/refs.ts` mined its two non-heading label sources with `[A-Za-z0-9_-]`, so `![Cap A](a.png){#fig-a.b}` was indexed as `fig-a` and a cell `#| label: fig-c.d` as `fig-c`, while quarto renders `id="fig-a.b"` / `id="fig-c.d"` and resolves `@fig-a.b` / `@fig-c.d` to them. ⚠ Session 220 had fixed the USE side, so those references were already READ correctly and still resolved to NULL — that session moved the failure rather than removing it, and said so when it filed this item.
- **⚠ The finding came from the corpus that scored 11 of 24, not the one that scored 36 of 36.** For Source 3 (an inline `{#fig-…}` block) the frozen prediction was pandoc's attribute-block set: **36/36**, a confirmation that taught nothing. For Source 2 (a `#| label:` cell option) the frozen prediction was *"the label is the YAML scalar verbatim"*: **11/24**. Chasing the 13 misses through the rendered HTML found the mechanism — quarto's **engine writes the label verbatim into a pandoc attribute block**, so `::: {#tbl-c09a$b .cell tbl-cap='Cap c09'}` appears as LITERAL TEXT in the output beside quarto's own *"The following string was found in the document: :::"* warning. Source 2 is Source 3 in a YAML costume; a second round written to discriminate that rule scored **9/9**.
- **The rule, measured rather than ported.** Pandoc's attribute identifier is a FLAT class — `[\p{L}\p{N}_:.-]` with **no follower rule and no position clause**: `{#fig-a..b}` is one id, and `{#fig-.x}`, `{#fig--y}`, `{#fig-a.}`, `{#fig-a:}` all define what they spell. ⚠ That is precisely where it differs from Session 220's use-side `citeKey`, which stops `@sec-a..b` at `sec-a`. Neither rule may be ported to the other, and each constant's docstring now names the question it answers.
- **⚠ A first-character clause neither the filed item nor the handoff mentioned,** and it was a different defect on each source: on Source 3 it sat AFTER the kind prefix and refused `{#fig-.x}` outright; on Source 2 it captures FROM the prefix, so it is vestigial and `#| label: fig-.d` had been indexing the bare `fig-` — a kind prefix with an empty name, a target no document can define. Found only by writing the RED per source rather than per rule.
- **⚠ Reverses an existing pin.** `refs.test.ts` C2 asserted `#| label: fig-plot.` → `fig-plot` on the rationale *"so it matches @ref usage"*. Measured false — quarto defines `fig-plot.` — and the truncated value was a label quarto never defines, which `@fig-plot` then navigated to. The correct id is itself unreachable (`@fig-plot.` consumes only `fig-plot`), which is the honest outcome: an index records definitions, a scanner records tokens, and truncating one until it matches the other is what invented the phantom.
- **Measurement.** 82 rows rendered through the real `quarto render` path (quarto 1.7.33), predictions frozen and hashed before every render. Adversarial corpus aimed at this implementation's own hypotheses: **2/9 → 6/9, INTRODUCED 0**. Document-level pre-vs-post sweep over **every field of every label across 46,510 documents** (113 tracked + 46,397 predecessor corpus): repo **0 movers**; corpus **1 mover**, and ⚠ **it is the deliverable itself** — Session 220's own rendered witness, with every column and every `sec-` label unchanged.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2163 passed / 66 files** (baseline 2135, +28) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (byte-identical to S180–S220) · `check-package` OK 42 files / 5.55 MB · `check-backlog` OK · `test:integration` **534 passing / exit 0** (baseline 532, +2), green first time on a go-ahead sought in advance. NOT RUN: `test:lsp` — no LSP surface touched.
- **TDD.** Guard block of **13 `it()`s** written and run GREEN before any production change, on this change's own polarity (a WIDENING) and on the fact that `INLINE_LABEL` is an unvalidated scan rather than a parsed block. **3 RED→GREEN cycles**, each RED confirmed to fail for the right reason. ⚠ GREEN 1 changed two clauses where RED 1 required one and the second was BACKED OUT to earn its own cycle. ⚠ A fourth RED came up green and is DISCLOSED as a demoted pin, not counted. Twelve further `it()`s are declared pins; provenance for all seven residual rows was established by probing the pre-session source, not by reading the diff.
- **Filed, not fixed (all three MEASURED and rendered):** `INLINE_LABEL` scans for the literal `{#`, so an id that is not FIRST in its block is invisible (`{.cls #fig-a03.x}` defines and resolves; this model indexes nothing) — the only lost-TP row of nine adversarial rows; the unvalidated scan indexes ids from brace groups quarto REFUSES (four shapes); and a SECOND adjacent block is indexed although only the first is the block.
- **Commits:** `38b39790` (1B claim, eight decision rules), `0bb69006` (guard), `443dd9cf` (C1), `3f2c6f9b` (C2), `68a724f9` (adversarial pins), `dba4d362` (integration).

### 2026-08-18 · [ad hoc] Session 220 — a cross-reference reaches an id holding `:`, `.` or a non-ASCII letter (SHIPPED)

- **Model:** Claude Opus 5.
- **The defect.** `src/core/refs.ts` held a NARROW use-side identifier set and a WIDE definition-side one. `# Methods {#sec-meth:ods}` was indexed correctly, `@sec-meth:ods` truncated to `sec-meth`, and `findLabel` returned null — go-to-definition dead on ids this model itself indexes right, while Session 218's own integration test asserts completion OFFERS that reference. Filed by Session 219 and ranked #1 by it.
- **⚠ Worse than filed, and measured before any code.** The completion surface did not truncate — it DIED: the backward scan stopped ON the colon, never reached the `@`, and returned null, so once an author typed a `:` they were offered nothing at all.
- **The rule, measured rather than ported.** 55 rows rendered through the real `quarto render` path (quarto 1.7.33), predictions frozen and committed before rendering: **38/40**, then a second round written to discriminate the surviving candidate rules, **15/15**. Pandoc's `citeKey` — *regchar, then any run of (regchar | internal punctuation IMMEDIATELY FOLLOWED BY a regchar)*. ⚠ Both round-1 misses are explained by that follower clause and by nothing else, and it is what stops `@sec-intro.` at `sec-intro` and breaks `@sec-a..b` at `sec-a`. Reusing Session 218's measured DEFINITION set would have been wrong on six rows — a different question with a different answer.
- **A second direction the filed item did not name.** Quarto resolves `@fig-plain.org` to nothing; the narrow rule returned `fig-plain` and navigated to it. The fix removes a FALSE navigation as well as a dead one.
- **⚠ A regression this session shipped TWICE and its own sweep caught.** Making the completion replace range "faithful" stranded a character on accept: `@sec-` lost its prefix hyphen (183 occurrences), then `@sec-my-` lost its name hyphen (34 more), so accepting `@sec-intro` produced `@sec-intro-`. Established as MINE, not inherited, by probing the pre-session source (`git archive` of the 1B commit) rather than by reading the diff. **`end` is not a parse claim** — it answers what the author has typed, not what pandoc consumes — so `refIdAt` stays exactly faithful and only `-` is permissive in the range.
- **Measurement.** Adversarial corpus (aimed at this implementation's own hypotheses, not the reader's behaviour): pre **5/15** → post **14/15**, INTRODUCED 0; the one row still disagreeing is the lookbehind item, filed. Sweep of BOTH surfaces at EVERY column of every line: **8,427,472** probes over 113 tracked documents, then **321,236,210** over 46,336 corpus documents. Final invariants: replace range **NEVER shrinks** (0, was 217), completion **never stops firing** (0), `start`/`typed` **never move** (0). The 6 rows where `refIdAt` returns null where it used to return an id are the FIX — quarto's token for `@sec-x-` is `sec-x` — and every column inside the token still resolves.
- **Verification.** `check-types` 0 · `check-types:unit` 0 · `compile` 0 · `compile-tests` 0 · `npm test` **2135 passed / 66 files** (baseline 2113, +22) · `test:oracle` 131 / 124 agree / 4 lost TP / 3 CARDINAL FP / 0 unrelated (byte-identical to S180–S219) · `check-package` OK 42 files / 5.55 MB · `check-backlog` OK · `test:integration` **532 passing / 0 failing / exit 0** (baseline 530, +2), both new tests watched BY NAME at log lines 87–88. NOT RUN: `test:lsp` — no LSP surface touched.
- **TDD.** Guard block of 8 `it()`s written and run GREEN before any production change, on this change's own polarity (a WIDENING). **4 RED→GREEN cycles**, each RED confirmed to fail for the right reason before its GREEN. Ten further `it()`s are declared PINS, not cycles.
- **Filed, not fixed (all three MEASURED and rendered):** the DEFINITION side truncates too, so `{#fig-a.b}` and `#| label: fig-c.d` are indexed under the wrong name while quarto defines and resolves both; the `@`-is-an-email test is a character class where pandoc uses `notAfterString`, wrong in both directions; and the completion scanner walks UTF-16 code units, so an astral letter reaches go-to-definition but not completion (pinned).
- **Commits:** `ec394ee9` (1B claim, eight decision rules), `38d8d449` (guard), `7af67047` (C1), `bf77372a` (C2), `f294b4bb` (C3), `fc0f15f9` (C4).
