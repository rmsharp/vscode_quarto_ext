...
| **`CELL_INFO` over-detection of glued malformed info strings** (`{python=x}`) | Low | ✅ **CLOSED Session 172** — `CELL_INFO` now carries quarto's own cell grammar (`[=A-Za-z]` + a `( *[ ,].*)?` tail), so `{python=x}` builds no cell and 6e mints no virtual document for it. *(Was: pre-existing shared-model gap, diagram-regions.ts + BACKLOG; 6e inherits it. Not a 6e spot-fix — cross-cutting, FM #18. Malformed input only.)* |
