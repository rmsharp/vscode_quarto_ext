...
This project's CHANGELOG: onboarding walkthrough + scaffolding commands, Sessions 49-51 only names `quarto.newDocument`/`quarto.createProject` — v1 stays scoped to exactly those two (no presentation/notebook commands), consistent with this project's own already-decided exclusions. Posit's fixed-template New Document is the right level of ambition to match (not exceed) for v1; this plan adds one small independent improvement (an optional title prompt, §5.1) that Posit's own doesn't have, which is a legitimate design choice, not a copy.
